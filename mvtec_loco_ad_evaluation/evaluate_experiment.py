...
def get_auc_spro_results(metrics: ThresholdMetrics,
    ...
    # --- NOT FROM ORIGINAL ---
    # To computer global AUSPRO
    auc_spro_global = get_auc_spros_for_metrics(metrics)
    auc_spro['global'] = auc_spro_global
    # --- NOT FROM ORIGINAL ---
...
def get_image_level_detection_metrics_aggregated(per_image_results):
    ...
    # --- NOT FROM ORIGINAL ---
    # To computer global AUROC as well
    all_anomalies = anomaly_scores['structural_anomalies'] + anomaly_scores['logical_anomalies']
    
    auc_roc_global = compute_classification_auc_roc(
            anomaly_scores['good'], all_anomalies)
            
    agg_results['auc_roc']['global'] = auc_roc_global
    # -------------------------
